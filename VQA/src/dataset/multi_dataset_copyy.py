    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                print(f"Error in __getitem__ at index {idx}: {type(e).__name__}: {e}")
                idx = random.randint(0, self.__len__() - 1)
...
class VQA_VQARad_Dataset(Dataset):
    def __init__(self, args, tokenizer, close_ended=True, mode="train", knowledge_encoder=False):
        ...
        if mode == "train":
            with open(os.path.join(self.data_root, "VQARAD", mode + ".json"), 'r') as f:
                self.data_list = json.load(f)
        elif mode == "validation":
            with open(os.path.join(self.data_root, "VQARAD", "val" + ".json"), 'r') as f:
                self.data_list = json.load(f)
        elif "test" in mode:
            with open(os.path.join(self.data_root, "VQARAD", mode + ".json"), 'r') as f:
                self.data_list = json.load(f)
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                #data = self.data_list.iloc[idx]
                #image_abs_path = os.path.join(self.args.data_root, self.data_list["img_path"])
                img_path = os.path.join(self.data_root, "VQARAD/VQA_RAD_Image_Folder", self.data_list[idx]["image_name"])
                image = np.array(Image.open(img_path).convert('RGB'))
                #minmax normalization
                # image = np.load(img_path)[np.newaxis, ...]  # nomalized
                image = self.transform(image)
