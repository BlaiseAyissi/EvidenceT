...
class BalSCL(nn.Module):
    ...
    def forward(self, centers1, features, targets, ):
        ...
        print("targets max:", targets.max().item())
        print("num_classes:", num_classes)

        assert (targets >= 0).all(), "Negative class index found!"
        assert (targets < num_classes).all(), "Class index out of range!"
        """
        
        device = torch.device('cuda:0')
                  #if features.is_cuda
                  #else torch.device('cpu'))
        batch_size = features.to(device).shape[0]
        ...
        # mask  
        mask = torch.eq(targets[:2 * batch_size], targets.T).to(device).float()
...
class LogitAdjust(nn.Module):
    ...
    def __init__(self, cls_num_list, tau=1, weight=None):
        ...
        cls_num_list = torch.tensor(cls_num_list, dtype=torch.float, device=torch.device('cuda:0'))
