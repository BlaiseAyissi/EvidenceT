...
def train_one_epoch(model,tokenizer, dataloader, num_batches, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        #text = tokenizer(text, context_length=256)
        #text = text.to(device=device)
        image = image.to(device=device)
        #image_class = tokenizer(image_class, context_length=256)
        ...
        if scaler is not None:
            ...
            if args.horovod:
                ...
                if getattr(args, "norm_gradient_clip", None) is not None:
...
def evaluate_one_epoch(model, tokenizer, dataloader, epoch, args):
    ...
    for sample in tqdm(dataloader):
        ...
        with torch.no_grad():
            image_features, text_features, ici_features, cls_logits, _, logit_scale = model(images_all, concatenated_text)
            n_rel = len(relation)
            n_head = len(head)
            n_def = len(head_def)
