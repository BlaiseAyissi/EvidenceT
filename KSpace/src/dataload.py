...
class UMLS_Dataset_Combine(Dataset):
    def __init__(self, args, knowledge_graph_path, transform,test=False):
        #read the json file
        slake = "/home/cv/Blaise/EvidenceT/KSpace/Datasets/Slake1.0/KG_Slake_Train.json"
        vqa = "/home/cv/Blaise/EvidenceT/KSpace/Datasets/VQARAD/KG_VQARAD_Train_map.json"
        slake_t = "/home/cv/Blaise/EvidenceT/KSpace/Datasets/Slake1.0/KG_Slake_Test.json"
        vqa_t = "/home/cv/Blaise/EvidenceT/KSpace/Datasets/VQARAD/KG_VQARAD_Test_map.json"
        ...
        if not test:
            slake_data = self._load_json(slake, source_tag = "slake")
            vqa_data = self._load_json(vqa, source_tag = "vqa")
            self.kg = slake_data + vqa_data
        else:
            slake_data = self._load_json(slake_t, source_tag = "slake")
            vqa_data = self._load_json(vqa_t, source_tag = "vqa")
            self.kg = slake_data + vqa_data
        ...
        self.num_intra_class_images = 36
        self.transform = transform
        self.test = test
        ...
        for item in self.kg:
            ...
            self.source_image_list.append(self._get_img_path(item))
            self.source_entity_list.append(item['head_entity'].lower())
            self.target_entity_list.append(item['tail_entity'].lower())
            self.edge_list.append(item['relation'].lower())
            self.source_entity_def_list.append(item['def_head'].lower())
